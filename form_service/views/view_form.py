...
class OwnerForm(Resource):
    ...
    def get(self, form_id):  # pylint: disable=no-self-use
        ...
        try:
            ...
            message = (jsonify({'message': 'Form with this id, by this user could not be found.'}),
                       400)
        output = FORM_SCHEMA.dump(form).data
        return jsonify(output) if form != [] else message
...
class EveryForm(Resource):
    ...
    def get(self, owner):  # pylint: disable=no-self-use
        ...
        try:
            ...
            message = jsonify({'message': 'Forms by this user could not be found.'}), 400
        output = FORMS_SCHEMA.dump(all_forms).data
        return jsonify(output) if all_forms != [] else message
